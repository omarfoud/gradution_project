# AI Job Backend - Azure Ready Final Version

This version is prepared for integration with the graduation project's Azure SQL backend.

## What this version includes

- FastAPI backend for job recommendation and analysis.
- FAISS + SQLite hybrid search over `job_descriptions.csv`.
- Azure SQL Server integration through `pyodbc`.
- Active CV retrieval by `user_id` from the project database.
- PDF/DOCX CV text extraction.
- Gemini analysis and career chat.
- Dockerfile with Microsoft ODBC Driver 18 for Azure SQL.
- `/health/app-db` endpoint to test Azure SQL connection.
...
`Resume.FilePath` should be an HTTP/HTTPS URL to a reachable PDF or DOCX file, preferably from Azure Blob Storage.
